...
import venture.cs414.android.monopoly.frontEnd.*;
...
public class GameController {
	...
	public GameController(int numPlayers, Context context) {
	...
	public String doAction(String action, String choice){
		switch (action) {
			case "sell property":
				sellAProperty(currentPlayer);
				break;
			case "mortgage":
				mortgageAProperty(currentPlayer, choice);
				break;
			case "unmortgage":

				break;
			case "buy house":
				buyAHouse(currentPlayer, choice);
				break;
			case "sell house":
				sellAHouse(currentPlayer, choice);
				break;
			case "buy hotel":
				buyAHotel(currentPlayer, choice);
				break;
			case "sell hotel":
				sellAHotel(currentPlayer, choice);
				break;
			case "finish turn":
				if(!currentPlayerHasMoved){
					return "You must roll before you can end your turn!";
				}
...
/*
	private String rollCurrentPlayer(){
		Property deed = null;
...
*/
	...
	public String rollCurrentPlayer(){
		...
		int diceRoll = dice.rollDice();
		boolean rolledDouble = dice.rolledDouble();
		String returnString = "You moved " + diceRoll + " spaces!";
		...
		if (rolledDouble) {
			...
			if (player.consecutiveTurns == 3) {
				player.putInJail();
				player.consecutiveTurns = 0;
				returnString = "You got caught speeding and went to jail!";
	...
	public List<String> getAvailableOptions(String action){
		List<String> options = new ArrayList<String>();
		switch (action) {
			case "sell property":
				options = getPropertiesCanMortgage(currentPlayer);
				break;
			case "mortgage":
				options = getPropertiesCanMortgage(currentPlayer);
				break;
			case "buy house":
				options = getStreetsThatCanBuyAHouse(getStreetsWithAllColors(currentPlayer));
				break;
			case "sell house":
				options = getStreetsThatCanSellAHouse(getStreetsWithHouses(currentPlayer));
				break;
			case "buy hotel":
				options = getStreetsThatCanBuyAHotel(getStreetsWithAllColors(currentPlayer));
				break;
			case "sell hotel":
				options = getStreetsThatCanSellAHotel();
				break;
			default:
				System.out.println("Unexpected action");
				break;
	...
	private void buyAHotel(Player player, String theProp){
		Street streetToBuyHotel = (Street) board.getPropertyByName(theProp);
	...
	private List<String> getStreetsThatCanSellAHotel(){
	...
	private void buyAHouse(Player player, String theProp){
		Street streetToBuyHouse = (Street) board.getPropertyByName(theProp);
	...
	private void sellAHotel(Player player, String theProp){
		Street street = (Street) board.getPropertyByName(theProp);
	...
	private void sellAHouse(Player player, String theProp){
		Street streetSellingHouse = (Street) board.getPropertyByName(theProp);
	...
	private void mortgageAProperty(Player player, String propName){
		Property mortgageProperty = board.getPropertyByName(propName);
	...
	private void unmortgageAProperty(Player player, String choice){
		Property propToUnmortgage = board.getPropertyByName(choice);
	}
	...
	public String getCurrentPlayerInfo(){
		String returnString = ("Player: " + currentPlayer.getName() + "\nToken: " + currentPlayer.getToken() + "\nPosition: "
				+ currentPlayer.getLocation() + "\nProperties: "
				+ currentPlayer.propertiesOwned.size() + "\n");
		return returnString;
	}
	...
	public boolean currentPlayerInJail(){
		return currentPlayer.getIsInJail();
	}
	...
	public void rollForJail(){
		...
		int turnsInJail = currentPlayer.turnsInJail;
		...
		currentPlayer.turnsInJail = turnsInJail+1;
	}

	public int turnsInJail(){
		return currentPlayer.turnsInJail;
	}
