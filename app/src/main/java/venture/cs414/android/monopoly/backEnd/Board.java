...
public class Board {
	...
	public Board(List<Player> players, Context context) {
		mContext = context;
	...
	private Boolean getXMLDoc() {
		...
		try {
			...
			xmlFileIS = mContext.getAssets().open(xmlFilename);
			//File xmlFile = new File("src/main/assets/" + xmlFilename);
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			monopolySpacesDoc = dBuilder.parse(xmlFileIS);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
